...
void DDA(int x1, int y1, int x2, int y2)
{
    ...
    for (int i = 0; i <= steps; i++)
    {
        ...
        {
            putpixel(round(int(X)), round(int(Y)), GREEN);
            delay(10);
        }
        else if (X - int(X) < 0.5 && Y - int(Y) >= 0.5)
        {
            putpixel(round(int(X)), round(int(Y) + 1), GREEN);
            delay(10);
        }
        else if (X - int(X) >= 0.5 && Y - int(Y) < 0.5)
        {
            putpixel(round(int(X) + 1), round(int(Y)), GREEN);
            delay(10);
        }
        else
        {
            putpixel(round(int(X) + 1), round(int(Y) + 1), GREEN);
            delay(10);
        }
